...
measurement="-D 180 --enable-pod-annotations "
...
csvfile="--csv-results-file ../results/results-mn-quick-${csv_suffix}-${csv_ts}.csv --csv-metrics-file ../results/metrics-mn-quick-${csv_suffix}-${csv_ts}.csv"
...
for iteration in `seq 1 ${iterations}`; do
  ...
  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-${tc_num}.${test_index}.log"
  ../../boatload/boatload.py ${dryrun} ${csvfile} --csv-title "${total_500_pods}n-1d-1p-1c-gu-${iteration}" ${metrics} -n ${total_500_pods} -d 1 -p 1 -c 1 -l -r --no-probes ${resources} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
  echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
  sleep ${sleep_period}
  echo "****************************************************************************************************************************************"
done
...
for iteration in `seq 1 ${iterations}`; do
  ...
  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-${tc_num}.${test_index}.log"
  ../../boatload/boatload.py ${dryrun} ${csvfile} --csv-title "${total_500_pods}n-1d-1p-1c-8cm-8s-${iteration}" ${metrics} -n ${total_500_pods} -d 1 -p 1 -c 1 -l -r -m 8 --secrets 8 --no-probes ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
  echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
  sleep ${sleep_period}
  echo "****************************************************************************************************************************************"
done
...
for iteration in `seq 1 ${iterations}`; do
  ...
  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-${tc_num}.${test_index}.log"
  ../../boatload/boatload.py ${dryrun} ${csvfile} --csv-title "${total_500_pods}n-1d-1p-1c-tcp-s1-l1-r1-${iteration}" ${metrics} -n ${total_500_pods} -d 1 -p 1 -c 1 -l -r ${probes} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
  echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
  sleep ${sleep_period}
  echo "****************************************************************************************************************************************"
done
...
for iteration in `seq 1 ${iterations}`; do
  ...
  echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} - ${total_500_pods} namespaces, 1 deploy, 1 pod, 1 container, gohttp image, 1 service, 1 route, ${probes}, no configmaps, no secrets, no resources set"
  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-${tc_num}.${test_index}.log"
  ../../boatload/boatload.py ${dryrun} ${csvfile} --csv-title "${total_500_pods}n-1d-1p-1c-http-s1-l1-r1-${iteration}" ${metrics} -n ${total_500_pods} -d 1 -p 1 -c 1 -l -r ${probes} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
  echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
  sleep ${sleep_period}
  echo "****************************************************************************************************************************************"
done
...
for iteration in `seq 1 ${iterations}`; do
  ...
  echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} - ${total_500_pods} namespaces, 1 deploy, 1 pod, 1 container, gohttp image, 1 service, 1 route, ${probes}, no configmaps, no secrets, no resources set"
  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-${tc_num}.${test_index}.log"
  ../../boatload/boatload.py ${dryrun} ${csvfile} --csv-title "${total_500_pods}n-1d-1p-1c-exec-s10-l10-r10-${iteration}" ${metrics} -n ${total_500_pods} -d 1 -p 1 -c 1 -l -r ${probes} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
  echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
  sleep ${sleep_period}
  echo "****************************************************************************************************************************************"
done
...
for iteration in `seq 1 ${iterations}`; do
  ...
  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-${tc_num}.${test_index}.log"
  ../../boatload/boatload.py ${dryrun} ${csvfile} --csv-title "${total_500_pods}n-1d-1p-1c-${iteration}" ${metrics} -n ${total_500_pods} -d 1 -p 1 -c 1 -l -r ${configmaps_secrets} ${probes} ${resources} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
  echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
  sleep ${sleep_period}
  echo "****************************************************************************************************************************************"
done
