...
for period in ${probe_periods}; do
  ...
  ../../boatload/boatload.py ${dryrun} ${csvfile} --csv-title "1n-1d-${total_pods}p-1c-gohttp-${period}" -n 1 -d 1 -p ${total_pods} -c 1 -l -m 0 --secrets 0 ${probes} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
  ...
  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-5.${test_index}.log"
  ../../boatload/boatload.py ${dryrun} ${csvfile} --csv-title "1n-${total_pods}d-1p-1c-gohttp-${period}" -n 1 -d ${total_pods} -p 1 -c 1 -l -m 0 --secrets 0 ${probes} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
  echo "$(date -u +%Y%m%d-%H%M%S) - node density 5.${test_index} complete, sleeping ${sleep_period}"
  sleep ${sleep_period}
  echo "****************************************************************************************************************************************"
  ...
  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-5.${test_index}.log"
  ../../boatload/boatload.py ${dryrun} ${csvfile} --csv-title "${total_pods}n-1d-1p-1c-gohttp-${period}" -n ${total_pods} -d 1 -p 1 -c 1 -l -m 0 --secrets 0 ${probes} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
  echo "$(date -u +%Y%m%d-%H%M%S) - node density 5.${test_index} complete, sleeping ${sleep_period}"
  sleep ${sleep_period}
  echo "****************************************************************************************************************************************"
done
