...
for iteration in `seq 1 ${iterations}`; do
  ...
  ../../boatload/boatload.py ${dryrun} ${qps_burst} ${csvfile} --csv-title "1n-1d-${total_pods}p-1c-${iteration}" -n 1 -d 1 -p ${total_pods} -c 1 -l -r --no-probes ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
...
for iteration in `seq 1 ${iterations}`; do
  ...
  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-${tc_num}.${test_index}.log"
  ../../boatload/boatload.py ${dryrun} ${qps_burst} ${csvfile} --csv-title "1n-${total_pods}d-1p-1c-${iteration}" -n 1 -d ${total_pods} -p 1 -c 1 -l -r --no-probes ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
  echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
  sleep ${sleep_period}
  echo "****************************************************************************************************************************************"
done
...
for (( arg_index=0; arg_index<${#images[@]}; arg_index++)); do
  for iteration in `seq 1 ${iterations}`; do
    ...
    logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-${tc_num}.${test_index}.log"
    ../../boatload/boatload.py ${dryrun} ${qps_burst} ${csvfile} --csv-title "${total_pods}n-1d-1p-1c-${csv_titles[$arg_index]}-${iteration}" -n ${total_pods} -d 1 -p 1 -c 1 -l -r --no-probes -i ${images[$arg_index]} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
    echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
    sleep ${sleep_period}
    echo "****************************************************************************************************************************************"
  done
...
for (( arg_index=0; arg_index<${#containers[@]}; arg_index++)); do
  for iteration in `seq 1 ${iterations}`; do
    ...
    logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-${tc_num}.${test_index}.log"
    ../../boatload/boatload.py ${dryrun} ${qps_burst} ${csvfile} --csv-title "${total_pods}n-1d-1p-${containers[$arg_index]}c-${iteration}" -n ${total_pods} -d 1 -p 1 -c ${containers[$arg_index]} -l -r --no-probes ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
    echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
    sleep ${sleep_period}
    echo "****************************************************************************************************************************************"
  done
