...
for obj_count in ${obj_counts}; do
  for iteration in `seq 1 ${iterations}`; do
    ...
    ../../boatload/boatload.py ${dryrun} ${qps_burst} ${csvfile} --csv-title "${total_pods}n-1d-1p-1c-${obj_count}cm-0s-${iteration}" -n ${total_pods} -d 1 -p 1 -c 1 -l -r -m ${obj_count} --secrets 0 --no-probes ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
...
for obj_count in ${obj_counts}; do
  for iteration in `seq 1 ${iterations}`; do
    ...
    logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-${tc_num}.${test_index}.log"
    ../../boatload/boatload.py ${dryrun} ${qps_burst} ${csvfile} --csv-title "${total_pods}n-1d-1p-1c-0cm-${obj_count}s-${iteration}" -n ${total_pods} -d 1 -p 1 -c 1 -l -r -m 0 --secrets ${obj_count} --no-probes ${resources} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
    echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
    sleep ${sleep_period}
    echo "****************************************************************************************************************************************"
  done
...
for obj_count in ${obj_counts}; do
  for iteration in `seq 1 ${iterations}`; do
    ...
    logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-${tc_num}.${test_index}.log"
    ../../boatload/boatload.py ${dryrun} ${qps_burst} ${csvfile} --csv-title "${total_pods}n-1d-1p-1c-${obj_count}cm-${obj_count}s-${iteration}" -n ${total_pods} -d 1 -p 1 -c 1 -l -r -m ${obj_count} --secrets ${obj_count} --no-probes ${resources} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
    echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
    sleep ${sleep_period}
    echo "****************************************************************************************************************************************"
  done
