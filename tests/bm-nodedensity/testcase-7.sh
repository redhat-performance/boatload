...
# Test Case 5 - Validate Max-pods with exec probes
set -e
set -o pipefail
...
for (( index=0; index<${#probes[@]}; index++)); do
  for iteration in `seq 1 ${iterations}`; do
    ...
    ../../boatload/boatload.py ${dryrun} ${qps_burst} ${csvfile} --csv-title "${total_pods}n-1d-1p-1c-${probes_csv_titles[$index]}-${iteration}" -n ${total_pods} -d 1 -p 1 -c 1 -l -r  ${probes[$index]} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
