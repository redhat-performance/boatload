...
measurement="-D 180"
csvfile="--csv-file tc1-${csv_suffix}-$(date -u +%Y%m%d-%H%M%S).csv"
...
for iteration in `seq 1 ${iterations}`; do
  ...
  echo "$(date -u +%Y%m%d-%H%M%S) - node density 1.${test_index} - ${iteration}/${iterations} - 1 namespace, 1 deploy, ${total_pods} pods, 1 container, gohttp image, 1 service, 1 route, no probes, no configmaps, no secrets, no resources set"
...
for iteration in `seq 1 ${iterations}`; do
  test_index=$((${test_index} + 1))
  echo "$(date -u +%Y%m%d-%H%M%S) - node density 1.${test_index} - ${iteration}/${iterations} - 1 namespace, ${total_pods} deploys, 1 pod, 1 container, gohttp image, 1 service, 1 route, no probes, no configmaps, no secrets, no resources set"
  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-1.${test_index}.log"
...
for (( arg_index=0; arg_index<${#images[@]}; arg_index++)); do
  for iteration in `seq 1 ${iterations}`; do
    test_index=$((${test_index} + 1))
    echo "$(date -u +%Y%m%d-%H%M%S) - node density 1.${test_index} - ${iteration}/${iterations} - ${total_pods} namespaces, 1 deploy, 1 pod, 1 container, ${tc_titles[$arg_index]} image, 1 service, 1 route, no probes, no configmaps, no secrets, no resources set"
    logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-1.${test_index}.log"
